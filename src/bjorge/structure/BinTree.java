...
public class BinTree {
    public static int[] nestMaks(int[] a)   // en turnering
    {
        ...
        //return new int[] {maksverdi,nestmaksverdi}; // størst og nest størst verdi

        //Oppgave 2
        int nestmaksIndeks = Integer.MIN_VALUE;
        int i = 2;
        boolean ferdig = false;
        while(!ferdig){
            if (b[i] == nestmaksverdi){
                nestmaksIndeks =i;
                ferdig = true;
            }
            i++;
        }
        return new int[] {1, nestmaksIndeks}; // størst og nest størst indeks
